    ...
    <v-main>
      <v-container>
        ...
        <v-sheet color="white" tag="section">
          <p class="text--secondary text-justify">{{ $vuetify.lang.t('$vuetify.idphotoSynopsis') }}</p>
        </v-sheet>
              ...
              <v-sheet class="d-rela" :style="{ width: staticImgWidth?staticImgWidth + 'px':'auto'}">
                ...
                <div class="before-img">
                  ...
                  <img
                    v-if="
                      files.length &&
                      files[files.length - 1]['response']['code'] === 200 &&
                      files[files.length - 1]['status'] &&
                      files[files.length - 1]['status']['code'] == 1
                    "
                    ref="staticImg"
                    :src="files[files.length - 1].status.wmk_url"
                    alt="before"
                  />
        ...
        <v-list three-line v-if="files.length" class="pic_list">
          <v-list-item v-for="item in c_b.cloneDeep(files).reverse()  " :key="item.id" style="border-bottom: 1px solid #c9cbce">
            ...
            <v-list-item-content style="padding: 35px 0 0;margin-top:-20px">
              <v-list-item-title>
              ...
              <v-list-item-subtitle class="text-caption font-weight-light d-flex align-center justify-between">
                <div style="font-family: Work Sans; font-style: normal; font-size: 14px; color: #999">
                  <span>
                    换背景（{{
                      { '-1:': '蓝', '255,255,255': '白', '85,142,213': '蓝色', '235,51,35': '红' }[item.data['rgb']] || '透明'
                    }}）
                  </span>
...
export default {
  ...
  metaInfo() {
    return {
      ...
      title: this.$vuetify.lang.t('$vuetify.idphotoName'),
      meta: [
        {
          name: 'keywords',
  ...
  watch: {
    ...
    files: {
      handler(value) {
        ...
        // let qian = ''
        // if (value) {
        //   let info = value[value.length - 1]
        //   if (info && info.response.mdfs) {
        //     duibih = 'https://sdkphoto.fangtangtv.com/api/toc/download/' + info.response.mdfs[0] + '.jpg'
        //     qian = info.blob
        //   }
        // }
